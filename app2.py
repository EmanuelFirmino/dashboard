...
if "BAIRRO" in cat_df.columns:
    ...
    if bairros_sel and "BAIRRO" in hist_df.columns:
...
# -----------------------
# HEATMAP & BARRAS (responsivo)
# -----------------------
cols = st.columns(2, gap="medium")
with cols[0]:
    df_heat = heat_df[(heat_df["EixoX"] == eixo_x) & (heat_df["EixoY"] == eixo_y)]
    if bairros_sel:
        df_heat = df_heat[df_heat["X_val"].isin(bairros_sel) | df_heat["Y_val"].isin(bairros_sel)]
    if df_heat.empty:
        ...
        df_heat = df_heat.groupby(["Y_val", "X_val"], as_index=False)["Quantidade"].sum()
        pivot = df_heat.pivot(index="Y_val", columns="X_val", values="Quantidade").fillna(0)

        heat = go.Figure(go.Heatmap(z=pivot.values, x=list(pivot.columns), y=list(pivot.index), colorscale="RdPu"))
...
total_real = int(cat_df["Quantidade"].sum())
